...
import os
class QuizG(): # mainly a class to host all user interface objects + some data (questions, players)
    def __init__(self):
    ...
    def _MenuWindow(self):
        ...
        newPButton = tk.Button(master=self._menuWindow, text="Create new player", fg='white',bg='goldenrod2',activebackground='gold',command = self._EnterName)
        newPButton.place(x = 50, y = 140)

        selectPButton = tk.Button(master=self._menuWindow, text="Select a player", fg='white' , bg='goldenrod2',command = self._SelectPWindow)
        selectPButton.place(x = 50, y = 180)

        viewStatsButton = tk.Button(master=self._menuWindow, text="View stats - under construction", fg='white' , bg='goldenrod2', command = None)
    ...
    def quit_game():
        ...
        if quit == 'ok':
            sys.exit(menuWindow.destroy()) 
        
    def show_result(self):
        global Name,score
        self._resWindow = tk.Toplevel(master = self._menuWindow) # new window
        top=self._resWindow
